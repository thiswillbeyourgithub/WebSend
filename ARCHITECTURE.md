...
└── src/
    ...
    └── public/             # Static frontend (vanilla HTML/CSS/JS, no build step)
        ...
        │   │               #   recently restarted and may be temporarily broken. The
        │   │               #   banner is only shown for restarts younger than 5 days
        │   │               #   (shouldShowMaintenanceBanner / MAINTENANCE_BANNER_MAX_AGE_MS);
        │   │               #   a long-running DEV instance shows the DEV badge but no
        │   │               #   banner. A missing serverStartedAt fails open (banner shown)
